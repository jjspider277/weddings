...
class NomencladoresRepository extends \Doctrine\ORM\EntityRepository{
    ...
    {
        $class = "backend\\ComunBundle\\Enums\\".$enumName;
        $enumsIds = array();
...
//         ldd($dir);
         ...
         $class = "backend\\ComunBundle\\Enums\\".$array[count($array)-1];
          $enumsIds = array();
        if($overwrite === false && class_exists($class))
            $enumsIds = $this->getEnumIds ($class);
     ...
     * @return type
     */
    public  function filterQB(\Doctrine\ORM\QueryBuilder $qb, $filters = array(),$resultType = ResultType::ObjectType,$order = null,$page = null)
    {
        $em = $this->getEntityManager();
...
//        $class = $this->getClassName();
    ...
    public function obtenerXArrayId($ids,$resultType = ResultType::ObjectType)
    {
        $qb = $this->createQueryBuilder('t')->andWhere('t.id in (:ids)')->setParameter('ids',$ids);
